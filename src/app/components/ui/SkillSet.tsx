...
export const SkillSet: FC<Technologies> = ({ title, stack }) => {
  return (
    <div className='border-2 border-c3-1 h-full flex flex-col rounded-lg'>
      ...
      <ul className='bg-c6 flex-1 p-6 flex flex-col gap-8'>
        {
          stack.map(({ name, asset }, i) => (
            <li key={`technology-${i}-${name}`} className='text-xl flex items-center gap-4'>
